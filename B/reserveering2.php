  ...
  <h1>Royalty</h1>
	...
	<div class="container">
		...
		<?php 
				...
				$thuisWinkel   	= $_POST["TW"];
				$opmerkingen    = $_POST["opmerking"];
				$haartype   	= $_POST["haartype"];
				$datum      	= $_POST["datum"];

				require_once "database.php";

				$sql = $conn->prepare("
										INSERT INTO reserveering VALUES
										(
										  :Id, :Naam, :Achternaam, :Leeftijd, :ThuisOfWinkel, :Opmerkingen, :Datum, :HaarType								  
										)
									");
				...
				$sql-> execute([
						...
						"Leeftijd"          => $leeftijd,
						"ThuisOfWinkel"     => $thuisWinkel,
						"Opmerkingen"       => $opmerkingen,
						"Datum"      	    => $datum,
						"HaarType"          => $haartype
						]);

				echo "De reserveering is gemaakt <br>";
				echo "<a href='main.html'> terug naar het menu </a>";
			   /*
			   //Dit is code wat ik voor zekerheid hier hou is van reserveeringen html
			   <label>wilt u thuis afspraak of in de winkel {thuis is ...$ meer}</label>
				 <select id="wilt u thuis afspraak of in de winkel {thuis is ...$ meer}" name="TW">
				 <option vulue="winkel">winkel</option>
				 <option vulue="thuis">thuis</option>
				  <br/>
			   */
		?>
		</h3>
