...
<main>
    <div id= "container">
        <div class="item">
        	<article>
        <h1>Partij toegevoegd</h1>
  <?php

                $id                 = NULL;
                $PartijeNaam        = $_REQUEST['PartijeNaam'];
                $Leden     			= $_REQUEST['Leden'];
                $JaarOpgericht      = $_REQUEST['JaarOpgericht'];
                $StaatVoor         	= $_REQUEST['StaatVoor'];
                $RechtsLinks        = $_REQUEST['RechtsLinks'];
                ...
                $sql = $conn->prepare("
                                        ...
                                        (
                                          :Id, :PartijeNaam, :Leden, :JaarOpgericht, :StaatVoor, :RechtsLinks                                
                                        )
                                    ");

                $sql-> execute([
                        "Id"                => $id,
                        "PartijeNaam"       => $PartijeNaam,
                        "Leden"    			=> $Leden,
                        "JaarOpgericht"     => $JaarOpgericht,
                        "StaatVoor"        	=> $StaatVoor,
                        "RechtsLinks"       => $RechtsLinks,
                        ]);

                echo "Uw Partij is aangemaakt <br>";
